...
public class CrystalCavesTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("crystal_caves_portal", Vec3i::compareTo, 300);
	public static PoiType poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("crystal_caves_portal",
				com.google.common.collect.Sets.newHashSet(
						ImmutableSet.copyOf(EssentialAdditionsModBlocks.CRYSTAL_CAVES_PORTAL.get().getStateDefinition().getPossibleStates())),
				0, 1).setRegistryName("crystal_caves_portal");
	...
	public CrystalCavesTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos p_77667_, Direction.Axis p_77668_) {
		...
		BlockState blockstate = EssentialAdditionsModBlocks.CRYSTAL_CAVES_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getExitPortal(entity, blockpos1, worldborder).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = CrystalCavesPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(),
						entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return CrystalCavesPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d, entity.getDimensions(entity.getPose()),
